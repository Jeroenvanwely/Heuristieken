...
def fold_check(grid, current_row, current_col, future_row, future_col, x):
    if future_row > current_row: 
        for i in range(1, len(protein) - x):
            if grid[future_row+i][future_col] != '_':
                return False

    elif future_row < current_row and p_list[x-1].row == p_list[x].row:
        for i in range(1, len(protein) - x):
            if grid[future_row-i][future_col] != '_':
                return False
        ...
        for i in range(1, len(protein) - x):
            if grid[future_row][future_col+i] != '_':
                return False

    elif future_col < current_col and p_list[x-1].column == p_list[x].column:
        for i in range(1, len(protein) - x):
            if grid[future_row][future_col-i] != '_':
                return False
...
def fold(future_row, future_col, row, col, p_list, x):
    ...
    elif future_row < row and p_list[x-1].row == p_list[x].row:
        for i in range(0, len(protein) - x):
            p_list[x+i].row = future_row-i
            p_list[x+i].column = future_col
